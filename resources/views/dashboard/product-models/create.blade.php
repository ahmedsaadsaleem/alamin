                    ...
                    <form method="POST" action="{{route('models.store')}}">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-lg-5 col-md-7">
                                <input class="form-control @error('model_name') is-invalid @else {{old('model_name') ? 'is-valid' : '';}} @enderror" type="text" name="model_name" id="model_name" value="{{old('model_name')}}" />
                                @error('model_name')
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-lg-5 col-md-7">
                                <select name="brand" id="brand" class="form-select @error('brand') is-invalid @else {{old('brand') ? 'is-valid' : '';}} @enderror">
                                    ...
                                    <option value="{{$brand->id}}" @selected(old('brand') == $brand->id)>{{$brand->brand_name}}</option>
                                    @endforeach
                                </select>
                                @error('brand')
                                    <div class="invalid-feedback">{{$message}}</div>
